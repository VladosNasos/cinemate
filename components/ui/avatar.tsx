...
export function AvatarMenu() {
  ...
  const handleSignOut = async () => {
    // немедленно почистить токены, затем редирект на /logout
    await logout();
    router.push("/logout");
  };
        ...
        <Popover.Content sideOffset={6} align="end" asChild>
          ...
          >
            ...
            <Popover.Close asChild>
              <NextLink href="/account" className="flex items-center gap-3 px-5 py-2 hover:bg-[#1a1a1a] transition-colors">
                <Avatar className="h-10 w-10">
                  <AvatarImage src="/placeholder.svg?height=40&width=40" alt="User" />
                  <AvatarFallback>N</AvatarFallback>
                </Avatar>
                <div className="leading-tight">
                  <p className="text-base font-semibold text-white">Nicolas Tetcher</p>
            ...
            {MENU_ITEMS.map((item, i) => {
              if (item.type === "divider")
                return <div key={i} className="border-t border-[#60605E]" />;

              // Special case: Sign out
              if (item.to === "/logout") {
                return (
                  <button
                    key={item.to}
